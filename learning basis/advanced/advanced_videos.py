...
def measure_curvature_real(
    ...
    lane_center_pixel = (left_fit_cr[0] + right_fit_cr[0]) * 720 ** 2 + (left_fit_cr[1] + right_fit_cr[1]) * 720 + (
                left_fit_cr[2] + right_fit_cr[2])

    # Calculate vehicle center in pixels
    vehicle_center_pixel = 1280 / 2

    # Calculate offset in pixels
    offset_pixel = lane_center_pixel - vehicle_center_pixel

    # Convert offset from pixels to meters
    offset_meter = offset_pixel * xm_per_pix

    return left_curverad, right_curverad, offset_meter
...
if __name__ == "__main__":
    ...
    while ret:
        ...
        left_curverad, right_curverad, offset_meter = measure_curvature_real(left_fit, right_fit, ploty)
        if offset_meter==0:
            print("车道线为直线：",offset_meter,"米")
        else:
            if offset_meter > 0:
                print("车道线向右偏移:", offset_meter, "米")
            else:
                print("车道线向左偏移:", abs(offset_meter), "米")
